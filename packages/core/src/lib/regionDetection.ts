...
export const detectUserRegion = async (): Promise<Region> => {
  ...
  const savedRegion = await storage.getItem('mikare_selected_region') as Region;
 ...
 */
export const getCurrentRegion = async (): Promise<Region> => {
  const now = Date.now();
 ...
 */
export const setUserRegion = async (region: Region): Promise<void> => {
 ...
 */
export const clearUserRegion = async (): Promise<void> => {
